...
import java.lang.Snippet.Template.Parameter;
...
public final class FormatterBuilder {
    ...
    /**
     * Create a specialized {@link MethodHandle} that will format a specific
     * set of data types, based on a {@link Formatter.FormatString} and locale.
     *
     * @param formatTemplate      a {@link FormattedText}
     * @param locale  {@link Locale} to use
     *
     * @return specialized {@link MethodHandle}
     */
    public static MethodHandle create(Snippet.Template<FormattedText> formatTemplate, Locale locale) {
        Objects.requireNonNull(formatTemplate, "st must not be null");
        Objects.requireNonNull(locale, "locale must not be null");
        String format = Formatter.stringTemplateFormat(formatTemplate.fragments());
        Class<?>[] ptypes = formatTemplate.parameters().stream()
                .map(Parameter::type)
                .toArray(Class<?>[]::new);
        MethodHandle mh = FormatterBuilder.create(format, locale, ptypes);
        return SnippetImpl.bindTo(formatTemplate, mh);
    }

    /**
     * Create a specialized {@link MethodHandle} that will format a specific
     * set of data types, based on a {@link Formatter.FormatString} and link Locale#ROOT}.
     *
     * @param formatterTemplate  a {@link FormattedText}
     *
     * @return specialized {@link MethodHandle}
     */
    public static MethodHandle create(Snippet.Template<FormattedText> formatterTemplate) {
        Objects.requireNonNull(formatterTemplate, "st must not be null");
        return create(formatterTemplate, Locale.getDefault());
    }
    ...
    private static String formatObject(Object object) {
        return (object instanceof Snippet<?> snippet &&
                snippet.template() instanceof FormattedText.Template template) ?
            String.format(template.cast(snippet)) :
...
//        }
    ...
    private MethodHandle build() {
        ...
        try {
            MethodHandle mh = StringConcatFactory.makeConcatWithTemplate(segments,
                List.of(ftypes));
            return filterArguments(mh, 0, filters);
    ...
    /**
     * Format the {@link Formatter.FormatString} from a cached {@link MethodHandle} if possible. Return null otherwise
     * so that an unoptimized method can be used.
     *
     * @param l   Locale
     * @param fs  associated {@link Formatter.FormatString}
    ...
    public static String format(Locale l, Snippet<FormattedText> fs) {
        MethodHandle formatterHandle = ((FormattedText.Template)fs.template()).formatterHandle(l);
        if (formatterHandle != null) {
            try {
                return (String)formatterHandle.invokeExact(fs);
            } catch (Throwable ex) {
