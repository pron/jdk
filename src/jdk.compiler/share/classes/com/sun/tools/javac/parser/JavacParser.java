...
/* ---------- parsing -------------- */
    ...
    JCExpression stringTemplate() {
        int oldmode = mode;
        accept(POUND);
        JCExpression snippetType = null;
        if (token.kind == IDENTIFIER) {
            selectTypeMode();
            snippetType = parseType();
        }
        if (token.kind != STRINGFRAGMENT &&
                token.kind != STRINGLITERAL) {
            // not a string template
            return syntaxError(token.pos, Errors.ExpectedStr("\""));
        }
        selectExprMode();
        ...
        JCExpression t = toP(F.at(pos).StringTemplate(snippetType, fragments, expressions, annotations));
        setMode(oldmode);
        return t;
    }
            ...
            switch (tk) {
                ...
                case RPAREN:
                    ...
                    switch (S.token(lookahead + 1).kind) {
                        ...
                        case DOUBLELITERAL: case CHARLITERAL: case STRINGLITERAL: case POUND:
                        case STRINGFRAGMENT:
                        case TRUE: case FALSE: case NULL:
    ...
    List<JCStatement> blockStatement() {
        ...
        case IDENTIFIER:
            if (token.name() == names.yield && allowYieldStatement) {
                ...
                switch (next.kind) {
                    case PLUS: case SUB: case STRINGLITERAL: case CHARLITERAL: case POUND:
                    case STRINGFRAGMENT:
                    case INTLITERAL: case LONGLITERAL: case FLOATLITERAL: case DOUBLELITERAL:
